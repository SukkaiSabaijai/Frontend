...
import { AllReviewResp, CreateReviewParams } from '@/modules/home/_types/home.type';
...
type Props = {
  ...
  const ReviewForm = ({markerReview,updateReview}:Props) => {
    ...
    const addReview = async() => {
        const createReviewParams:CreateReviewParams = {markerId:JSON.stringify(markerReview.markerId),rating:JSON.stringify(newRating),review:newComment}
      ...
      <div className="p-4 w-full bg-blue-100 rounded-lg shadow-md relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-1 text-yellow-500 text-xl font-bold">
            ...
            <span className="text-gray-600 text-xl font-bold">({markerReview.reviewCount} reviews)</span>
          </div>
        </div>
        {isButtonVisible && (
          <button
            onClick={() => {setIsFloating(true),setIsButtonVisible(false)}}
            className="p-2 bg-custom-blue text-white rounded-full fixed bottom-5 right-5 hover:bg-custom-yellow z-50"
          >
            Add Review
          </button>
          )}
